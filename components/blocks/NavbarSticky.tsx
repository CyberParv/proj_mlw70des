...
export default function NavbarSticky({ logo, navItems, ctaLabel, ctaHref }: NavbarStickyProps) {
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {ctaLabel && ctaHref && (
            <Button className="font-bold uppercase tracking-wide" asChild><a href={ctaHref}>{ctaLabel}</a></Button>
          )}
        </nav>
        <button
      ...
      {mobileOpen && (
        <div className="border-t bg-background md:hidden">
          <nav className="container mx-auto flex flex-col gap-4 px-4 py-4">
            ...
            {ctaLabel && ctaHref && (
              <Button className="w-full font-bold uppercase tracking-wide" asChild><a href={ctaHref}>{ctaLabel}</a></Button>
            )}
          </nav>
        </div>
      )}
